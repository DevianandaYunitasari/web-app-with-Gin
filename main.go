...
func RunServer(db *gorm.DB, gin *gin.Engine) *gin.Engine {
	...
	task := gin.Group("/task")
	{
		...
		task.GET("/category/:id", apiHandler.TaskAPIHandler.GetTaskListByCategory) // TODO: answer here
	}
	...
	category := gin.Group("/category")
	{
		...
		category.GET("/list", apiHandler.CategoryAPIHandler.GetCategoryList) // TODO: answer here
	}
